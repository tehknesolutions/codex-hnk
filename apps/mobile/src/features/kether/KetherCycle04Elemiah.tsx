...
export function KetherCycle04Elemiah() {
  ...
  const [fragmentsLit, setFragmentsLit] = useState(3);
  const [displayDay, setDisplayDay] = useState(16);
  ...
  const refresh = useCallback(async () => {
    ...
    if (!error && data) {
      ...
      if (data.current_day >= 16 && data.current_day <= 20) setDisplayDay(data.current_day);
      if (data.current_day > 20) setDisplayDay(20);
    }
    ...
    try {
      ...
      // Crown is presentation-only here; canonical progression remains server-owned.
    }
  ...
  if (loading && auth.phase === 'signed-in') {
    return (
      <View style={styles.loading}>
        ...
        <Text style={styles.loadingText}>ABRINDO A VOZ DE ELEMIAH</Text>
      </View>
    );
  }

  const completed = Math.max(0, Math.min(5, currentDay - 16));

  return (
    <View style={styles.screen}>
      <View style={styles.rail}>
        <View style={styles.railLine} />
        {[16, 17, 18, 19, 20].map((day, index) => {
          ...
          return (
            <Pressable key={day} disabled={!available} onPress={() => setDisplayDay(day)} style={styles.nodeWrap}>
              ...
              <Text style={[styles.nodeLabel, active && styles.nodeLabelActive]}>
                {day === 16 ? 'PERCEBER' : day === 17 ? 'VOCALIZAR' : day === 18 ? 'RESSONAR' : day === 19 ? 'MEDIR' : 'SILENCIAR'}
              </Text>
            </Pressable>
          );
        })}
      ...
      <View style={styles.header}>
        <View>
          <Text style={styles.eyebrow}>KETHER · CICLO IV · A VOZ DE ELEMIAH</Text>
          <Text style={styles.heading}>DIA {String(displayDay).padStart(3, '0')}</Text>
        </View>
        <View style={styles.stats}>
          <Text style={styles.stat}>{title.toUpperCase()}</Text>
          <Text style={styles.stat}>{xpTotal} XP</Text>
