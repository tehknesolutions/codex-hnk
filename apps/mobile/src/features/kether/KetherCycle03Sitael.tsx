...
import { SitaelDays011to015Experience } from './SitaelDays011to015Experience';

export function KetherCycle03Sitael() {
  const auth = useHnkAuth();
  const [currentDay, setCurrentDay] = useState(11);
  const [xpTotal, setXpTotal] = useState(0);
  const [title, setTitle] = useState('Neófito');
  const [fragmentsLit, setFragmentsLit] = useState(2);
  const [displayDay, setDisplayDay] = useState(11);
  ...
  const refresh = useCallback(async () => {
    ...
    if (!error && data) {
      ...
      if (data.current_day >= 11 && data.current_day <= 15) setDisplayDay(data.current_day);
      if (data.current_day > 15) setDisplayDay(15);
    }
    try {
  ...
  if (loading && auth.phase === 'signed-in') return <View style={styles.loading}><ActivityIndicator color="#efe0a2" /><Text style={styles.loadingText}>ABRINDO A DISCIPLINA DE SITAEL</Text></View>;

  const completed = Math.max(0, Math.min(5, currentDay - 11));
  return (
    <View style={styles.screen}>
        ...
        {[11, 12, 13, 14, 15].map((day, index) => {
          ...
          return (
            <Pressable key={day} disabled={!available} onPress={() => setDisplayDay(day)} style={styles.nodeWrap}>
              <View style={[styles.node, done && styles.nodeDone, active && styles.nodeActive, !available && styles.nodeLocked]}><Text style={[styles.nodeText, (done || active) && styles.nodeTextActive]}>{done ? '✓' : day}</Text></View>
              <Text style={[styles.nodeLabel, active && styles.nodeLabelActive]}>{day === 11 ? 'OBSERVAR' : day === 12 ? 'SUSTENTAR' : day === 13 ? 'CORRIGIR' : day === 14 ? 'DIRIGIR' : 'RETORNAR'}</Text>
            </Pressable>
          );
        })}
      </View>
      <View style={styles.header}>
        <View><Text style={styles.eyebrow}>KETHER · CICLO III · A DISCIPLINA DE SITAEL</Text><Text style={styles.heading}>DIA {String(displayDay).padStart(3, '0')}</Text></View>
      ...
      <SitaelDays011to015Experience day={displayDay as 11 | 12 | 13 | 14 | 15} />
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  nodeLabel: { color: '#51535a', fontSize: 6, letterSpacing: 0.7 },
