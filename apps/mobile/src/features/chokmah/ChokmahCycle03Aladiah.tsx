...
export function ChokmahCycle03Aladiah() {
  ...
  const refresh = useCallback(async () => {
    ...
    if (!error && data) {
      ...
      if (data.current_day >= 47 && data.current_day <= 51) setDisplayDay(data.current_day as AladiahDay);
      if (data.current_day > 51) setDisplayDay(51);
    }
    setLoading(false);
  ...
  const completed = Math.max(0, Math.min(5, currentDay - 47));
  ...
  return (
    <View style={styles.screen}>
        ...
        {ALADIAH_DAYS.map((day, index) => {
          ...
          return (
            <Pressable key={day} disabled={!available} onPress={() => setDisplayDay(day)} style={styles.nodeWrap}>
              ...
              <Text style={[styles.nodeLabel, active && styles.nodeLabelActive]}>{day === 47 ? 'PERIFERIA' : day === 48 ? 'LOOPS' : day === 49 ? 'PULSO' : day === 50 ? 'CONTORNO' : 'SIGILO'}</Text>
            </Pressable>
          );
        })}
      </View>
      ...
      <View style={styles.header}>
        <View><Text style={styles.eyebrow}>CHOKMAH · CICLO III · DISCERNIMENTO DE ALADIAH</Text><Text style={styles.heading}>DIA {String(displayDay).padStart(3, '0')}</Text></View>
      ...
      {displayDay === 51 ? <AladiahDay051SigilExperience /> : null}
    </View>
  );
}
