...
/**
 * Portal073 is intentionally routed before publication so the approved operator
 * package is inspectable on mobile without exposing begin/seal/XP actions.
 * This component must remain fail-closed while PORTAL073_PRODUCTION_ENABLED=false.
 */
export function Day073PortalReadinessMobile(){
  ...
  return <ScrollView contentContainerStyle={s.shell}>
    <Text style={s.kicker}>CHOKMAH → BINAH · PORTAL 073</Text>
    <Text style={s.title}>O Voo do Mago</Text>
    ...
    <Card title="Operadores V1">
      <Text style={s.text}>Tuner · {PORTAL073_TUNER_ID}</Text>
      <Text style={s.text}>ACTIVE · {PORTAL073_TRANSITION_PRESET_ID} · 528/532 Hz · 600 s</Text>
      <Text style={s.text}>Sigilo · {PORTAL073_SIGIL_ID}</Text>
      <Text style={s.mono}>SHA-256 {MAGICIAN_MERCURY_SIGIL_SHA256}</Text>
