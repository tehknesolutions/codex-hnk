...
function asBufferSource(bytes:Uint8Array):BufferSource{return bytes as unknown as BufferSource}
async function checksum(input:{nonce:string;ciphertext:string;aad:string}){const bytes=new TextEncoder().encode(`${input.nonce}.${input.ciphertext}.${input.aad}`);return hex(new Uint8Array(await crypto.subtle.digest('SHA-256',asBufferSource(bytes))))}
...
export function createWebMediaVaultPort():VaultMediaPort{return{
 ...
 async saveMedia(input){if(input.kind!=='audio')throw new Error('media_vault_kind_unsupported');validate(input);const key=await getKey(input.userId);const nonce=crypto.getRandomValues(new Uint8Array(12));const aad=JSON.stringify({schema:'hnk-media-vault-v1',user:input.userId,day:input.day,kind:'audio',mimeType:input.mimeType,durationSeconds:Number(input.durationSeconds.toFixed(3))});const aadBytes=new TextEncoder().encode(aad);const sealed=await crypto.subtle.encrypt({name:'AES-GCM',iv:asBufferSource(nonce),additionalData:asBufferSource(aadBytes),tagLength:128},key,asBufferSource(new Uint8Array(input.bytes)));const payload={nonce:b64(nonce),ciphertext:b64(new Uint8Array(sealed)),aad};const localRef=`local-media-vault:${crypto.randomUUID()}`;const record:MediaRecord={localRef,userId:input.userId,day:input.day,kind:'audio',mimeType:input.mimeType,byteLength:input.bytes.byteLength,durationSeconds:Number(input.durationSeconds.toFixed(3)),...payload,checksumSha256:await checksum(payload),createdAt:new Date().toISOString()};await putMedia(record);return mediaRef(record)},
 async loadMedia(userId,localRef){const record=await getMedia(localRef);if(!record||record.userId!==userId)throw new Error('media_vault_entry_not_found');if(await checksum(record)!==record.checksumSha256)throw new Error('media_vault_checksum_mismatch');const key=await getKey(userId);const nonce=unb64(record.nonce);const aadBytes=new TextEncoder().encode(record.aad);const ciphertext=unb64(record.ciphertext);const plain=await crypto.subtle.decrypt({name:'AES-GCM',iv:asBufferSource(nonce),additionalData:asBufferSource(aadBytes),tagLength:128},key,asBufferSource(ciphertext));return{mimeType:record.mimeType,bytes:new Uint8Array(plain),durationSeconds:record.durationSeconds}},
