...
function decodeIChing(binary) {
  ...
  return Object.freeze({
    ...
    primary: Object.freeze({
      binaryBottomToTop: primaryLines.join(''),
      binaryIndex: bottomFirstBinaryIndex(primaryLines),
      lowerTrigram: primaryLower,
      upperTrigram: primaryUpper,
      kingWen: kingWenNumber(primaryLower.id, primaryUpper.id)
    }),
    resulting: Object.freeze({
      binaryBottomToTop: transformedLines.join(''),
      binaryIndex: bottomFirstBinaryIndex(transformedLines),
      lowerTrigram: transformedLower,
      upperTrigram: transformedUpper,
      kingWen: kingWenNumber(transformedLower.id, transformedUpper.id)
    })
  });
}

function digitalSum(value) {
...
function triangularIndex(value) {
  const n = (Math.sqrt(8 * value + 1) - 1) / 2;
  return Number.isInteger(n) ? n : null;
}

function hexColorFromRgb(rgb) {
  return `#${rgb.map(value => value.toString(16).padStart(2, '0').toUpperCase()).join('')}`;
}
...
function decodeColors(binary) {
  ...
  return Object.freeze({
    essence: hexColorFromRgb(rgb),
    shadow: hexColorFromRgb(shadow),
    manifestation: hexColorFromRgb(manifestation),
    essenceRgb: Object.freeze(rgb),
    derivation: Object.freeze({ shadow: '0xFFFFFF XOR ESSENCE', manifestation: 'RGB->GBR byte rotation' })
  });
}
...
export function runOracle(input) {
  ...
  const result = {
    ...
    hnk: Object.freeze({
      glyphId,
      phonemeIpa: glyph.phonemeIpa,
      worldId: glyph.worldId,
      runtimeVisualState: glyph.visualState,
      selection: glyphPick,
      sourceBits: 'B017-B022',
      authority: '@hnk/glyphs'
    }),
    ...
    astrology: Object.freeze({
      zodiac: ZODIAC[zodiacPick.value],
      zodiacIndex: zodiacPick.value + 1,
      zodiacSelection: zodiacPick,
      planet: CLASSICAL_PLANETS[planetPick.value],
      planetIndex: planetPick.value + 1,
      planetSelection: planetPick,
      element: ELEMENTS[elementIndex],
      elementIndex: elementIndex + 1
    }),
    alchemy: Object.freeze({
      principle: ALCHEMICAL_PRINCIPLES[alchemyPrinciplePick.value],
      principleSelection: alchemyPrinciplePick,
      phase: ALCHEMICAL_PHASES[phaseIndex]
    }),
    numerology: Object.freeze({
      raw: numberRaw,
      digitalSum: digitalSum(numberRaw),
      digitalRoot: digitalRoot(numberRaw),
      binary: numberRaw.toString(2),
      hex: numberRaw.toString(16).toUpperCase(),
      parity: numberRaw % 2 === 0 ? 'EVEN' : 'ODD',
      isPrime: isPrime(numberRaw),
      primeFactors: Object.freeze(primeFactors(numberRaw)),
      triangularIndex: triangularIndex(numberRaw)
    }),
...
export function assertRuntimeGlyphCoverage() {
  if (HNK40_GLYPH_IDS.length !== 40) throw new Error(`@hnk/glyphs cardinality drift: ${HNK40_GLYPH_IDS.length}`);
  return true;
}
