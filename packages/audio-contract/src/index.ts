...
export function validateHnkAudioPreset(preset: HnkAudioPreset): string[] {
  ...
  for (const layer of preset.layers) {
    ...
    if ((layer.kind === 'carrier' || layer.kind === 'ritual-tone') && !finitePositive(layer.hz)) {
      errors.push(`${layer.kind}.hz must be > 0`);
    }
    if (layer.kind === 'binaural') {
      ...
      if (Math.abs(Math.abs(layer.rightHz - layer.leftHz) - layer.differenceHz) > 0.0001) {
  ...
  if (preset.status === 'published' && !/^[a-f0-9]{64}$/i.test(preset.renderChecksumSha256 ?? '')) {
...
export const DAY002_AUDIO_PRESET_V1: HnkAudioPreset = Object.freeze({
  ...
  sourceReferences: [
    {
      id: 'D002-CANON-528',
      rawLabel: 'áudio binaural em frequência Solfeggio de 528Hz',
      sourceRef: 'Tehkne-Solutions/hnk-codex-365:canon/capitulo-01-kether/dia-002.md',
      role: 'ritual-tone',
      hz: 528,
      note: 'Canon wording is preserved; the product does not reinterpret 528 as the binaural difference.',
    },
    {
      id: 'D002-HNK-BASE-432',
      rawLabel: '432 Hz base',
      sourceRef: 'docs/audio/HNK_DAY002_AUDIO_FREEZE_V1.md',
      role: 'carrier-base',
      hz: 432,
      note: 'HNK product decision V1.',
    },
    {
      id: 'D002-HNK-THETA-6',
      ...
      hz: 6,
      note: 'Product mapping only; no claim that playback causes a neurological Theta state.',
    },
  ],
...
export function createDay002AudioLoopWavBase64(): string {
  return bytesToBase64(createDay002AudioLoopWavBytes());
}

export const HNK_AUDIO_CONTRACT_VERSION = '1.0.0' as const;
