...
export function validateCycle1LanguageCoverage() {
  ...
  if (HNK_CYCLE1_AUTHORED_CANDIDATES.length !== 3) errors.push(`Expected 3 governed authored candidates, got ${HNK_CYCLE1_AUTHORED_CANDIDATES.length}`);
  if (JSON.stringify(HNK_CYCLE1_AUTHORED_CANDIDATES.map((entry) => entry.transliteration)) !== JSON.stringify(['KUVAN','VALA','KUON'])) errors.push('Authored candidate list drift');
  if (governedUniqueLanguageAssets !== 34) errors.push(`Expected 34 governed unique language assets, got ${governedUniqueLanguageAssets}`);
  ...
  const expectedCandidateCounts = {L01:3,L02:0,L03:0,L04:0,L05:0,L06:0,L07:0};
  ...
  if (JSON.stringify(l01.authoredCandidateIds) !== JSON.stringify(['AUTH-001','AUTH-002','AUTH-003'])) errors.push('L01 authored candidate provenance drift');
  if (HNK_CYCLE1_LANGUAGE_GATE.recoveredProxyRatio !== 0.2153) errors.push('Recovered proxy ratio drift');
  if (HNK_CYCLE1_LANGUAGE_GATE.governedAssetProxyRatio !== 0.2361) errors.push('Governed asset proxy ratio drift');
