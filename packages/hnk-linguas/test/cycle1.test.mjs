...
const authoredForms = ['KUVAN','VALA','KUON','NE','BIZO','DUVE','HOYU','KETI','LUSO','MUPI','NURA','PEVU','TOMI','ZOKA'];
const authoredIds = Array.from({length:14},(_,i)=>`AUTH-${String(i+1).padStart(3,'0')}`);
...
test('recovered registry and authored candidates remain separate metrics', () => {
  ...
  assert.equal(HNK_CYCLE1_LANGUAGE_GATE.authoredCandidateForms, 14);
  assert.equal(HNK_CYCLE1_LANGUAGE_GATE.governedUniqueLanguageAssets, 45);
  ...
  assert.equal(HNK_CYCLE1_LANGUAGE_GATE.governedAssetProxyGap, 99);
  assert.equal(HNK_CYCLE1_LANGUAGE_GATE.governedAssetProxyRatio, 0.3125);
...
test('lesson binding counts preserve recovered provenance and governed additions', () => {
  ...
  assert.deepEqual(Object.fromEntries(HNK_CYCLE1_LANGUAGE_COVERAGE.map((lesson) => [lesson.lessonId,lesson.authoredCandidateCount])), {L01:14,L02:0,L03:0,L04:0,L05:0,L06:0,L07:0});
  ...
  assert.equal(l01.languageAssetCount, 24);
