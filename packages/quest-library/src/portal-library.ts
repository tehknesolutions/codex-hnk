...
export class PortalAwareQuestLibrary implements RuntimeQuestBundleLoader,QuestDefinitionLoader{private readonly base=new Day001To035Library();async loadBundle(day:number):Promise<RuntimeQuestBundle|null>{if(day===36||day===37||day===38||day===39||day===40||day===41||day===42||day===43)return validate(bundle(day));return this.base.loadBundle(day)}async loadDay(day:number):Promise<QuestDefinition|null>{return(await this.loadBundle(day))?.quest??null}listAvailableDays():number[]{return[...new Set([...this.base.listAvailableDays(),36,37,38,39,40,41,42,43])].sort((a,b)=>a-b)}}
