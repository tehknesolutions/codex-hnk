...
function validateBundle(bundle: RuntimeQuestBundle): RuntimeQuestBundle {
  ...
  if (bundle.quest.canonical.source_sha !== bundle.canon.source.blob_sha) throw new Error(`quest_library_source_sha_mismatch:${bundle.day}`);
  return bundle;
}
...
export class BundledQuestLibrary implements RuntimeQuestBundleLoader, QuestDefinitionLoader {
  ...
  async loadDay(day: number): Promise<QuestDefinition | null> { return (await this.loadBundle(day))?.quest ?? null; }
  listAvailableDays(): number[] { return [...BUNDLES.keys()].sort((a, b) => a - b); }
}
