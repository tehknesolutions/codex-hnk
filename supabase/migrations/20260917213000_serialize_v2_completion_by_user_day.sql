-- Restore explicit V2 serialization by authenticated user + canonical day.
-- Preserves client_completion_id replay locking and all existing progression rules.
-- Does NOT activate Portal073, publish operators, award XP, or unlock Day074.

-- Portal073 completion may eventually award its canonical XP, but it MUST NOT
-- unlock/autostart Day074 or promote Binah through the generic V2 path.
-- Publication remains independently fail-closed because Day073 has no ACTIVE
-- completion contract until the authoritative release gate is opened.
...
begin
  ...
  -- Serialize all completion attempts for the same authenticated user/day,
  -- even when concurrent clients use different client_completion_id values.
  -- Keep the client-id lock as the narrower replay/conflict guard.
  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(v_uid::text || ':day:' || p_day::text, 0)
  );
  perform pg_catalog.pg_advisory_xact_lock(
  ...
  if not v_existing then
    ...
    if v_new_completion then
      ...
      if p_day = 73 then
        -- Portal073 is a terminal/frozen boundary for the current release.
        -- XP accounting is allowed only after an ACTIVE Day073 contract exists,
        -- but generic Day074/Binah progression is deliberately suppressed.
...
-- Internal privileged implementation only. Public callers continue through
-- public.complete_codex_day_v2 (security invoker), which enforces auth.uid().
revoke execute on function hnk_private.complete_codex_day_v2_impl(smallint,uuid,text,text,text,text,text,timestamptz)
