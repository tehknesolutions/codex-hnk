-- Serialize canonical completion attempts per (user_id, day) before reading completion state.
-- This complements the existing UNIQUE(user_id, day) and XP idempotency key,
-- and prevents two different Practice Sessions for the same user/day from both
-- observing a pre-completion state concurrently.

create or replace function public.complete_codex_day(
  p_day smallint,
  p_session_id uuid,
  p_local_record_hash text default null::text,
  p_client_completed_at timestamptz default null::timestamptz
) returns jsonb
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  -- Transaction-scoped lock shared by every Practice Session for this user/day.
  -- The second concurrent request cannot evaluate v_existing until the first
