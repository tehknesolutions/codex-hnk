-- Mirrors applied Supabase migration: 20260909013809_fix_atziluth_import_run_source_repo
-- Fixes provenance metadata for successor canonical syncs without changing canonical day content.
...
begin
  ...
  insert into public.codex_import_runs(id, source_sha, source_repo, days_imported, status)
  values (v_import_id, p_source_commit_sha, v_repo, 0, 'started');
...
update public.codex_import_runs r
set source_repo = 'tehknesolutions/codex-hnk'
where r.source_repo = 'Tehkne-Solutions/hnk-codex-365'
  and exists (
    select 1
    from public.codex_days d
    where d.content->>'source_kind' = 'successor'
      and d.content->>'source_commit_sha' = r.source_sha
  );
