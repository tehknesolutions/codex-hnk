...
const d1 = loadJson('docs/experience/kether/day-001/day-001.quest.json');
const d2 = loadJson('docs/experience/kether/day-002/day-002.quest.json');
...
for (const definition of definitions) {
  ...
  days.add(definition.day);
  ids.add(definition.id);
}

if (d1.day !== 1 || d2.day !== 2) fail('Day 001/002 catalog ordering drift');
...
if (/HNK-KETHER-D00[12]/.test(registry + catalog)) fail('engine registry/catalog hardcodes Day 001/002 ids');
if (/day\s*===\s*[12]/.test(registry + catalog)) fail('engine registry/catalog contains day-specific branching');

const rendererTypes = new Set(Object.keys(d1Renderer.phase_renderers ?? {}));
for (const phase of [...d1.phases, ...d2.phases]) {
  if (!rendererTypes.has(phase.type)) fail(`renderer V1 does not cover ${phase.type}`);
}
...
if (!process.exitCode) {
  console.log('QUEST CATALOG PASS: Day 001 + Day 002 resolve through one generic registry/catalog; no day-specific engine branch');
}
