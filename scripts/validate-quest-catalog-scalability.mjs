...
if([...days].sort((a,b)=>a-b).join(',')!==expectedDays.join(','))fail('Day 001-008 catalog coverage drift');
...
if(/HNK-KETHER-D00[1-8]/.test(registry+catalog))fail('engine registry/catalog hardcodes Day 001-008 ids');
if(/day\s*===\s*[1-8]/.test(registry+catalog))fail('engine registry/catalog contains day-specific branching');
...
if(!process.exitCode)console.log('QUEST CATALOG PASS: Days 001-008 resolve through one generic registry/catalog; engine phase types cover every configured phase; no day-specific engine branch');
