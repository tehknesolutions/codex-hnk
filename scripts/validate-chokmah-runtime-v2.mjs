...
check('Haziel runtime definitions extend continuously through Day 045', ['HAZIEL_DAY_042','HAZIEL_DAY_043','HAZIEL_DAY_044','HAZIEL_DAY_045'].every((x) => has(hazielDefs, x)) && !has(hazielDefs, 'HAZIEL_DAY_046'));
check('Haziel cycle mounts 042-045 runtime', has(hazielCycle, 'HazielDays042to044Experience') && has(hazielCycle, 'HazielDay045AudioExperience') && has(hazielCycle, 'displayDay === 45'));
check('Day 046 is now an explicit G7 runtime gap rather than sequence blocker', has(hazielCycle, 'CANONICAL_STORAGE_READY · G7_RUNTIME_PENDING') && !has(hazielCycle, 'SEQUENCE_BLOCKED_BY_DAY_045') && !has(hazielDefs, 'HAZIEL_DAY_046'));
...
check('canonical successor contains Haziel 042-046 including Day 045', [42,43,44,45,46].every((d) => exists(`content/canon/atziluth/chokmah/dia-${String(d).padStart(3,'0')}.md`)));
...
console.log('PASS CHOKMAH-V2 · Cahetel 037-041 + Haziel 042-045 runtime, Day046 explicit G7 gap and server contracts valid');
