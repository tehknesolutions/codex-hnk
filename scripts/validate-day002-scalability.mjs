...
if (pack.version !== '0.5.0') fail('Quest Pack version drift');
...
if (pack.qa?.code_integration !== 'STATIC_GATES_VERSIONED' || pack.qa?.ci_is_release_evidence !== false) fail('Quest Pack QA boundary drift');
if (pack.qa?.browser_runtime !== 'PENDING' || pack.qa?.expo_device_runtime !== 'PENDING') fail('device/browser QA must remain pending');
...
if (pack.proof_result !== 'PASS_ARCHITECTURE_REUSE__PROGRESSION_FROZEN__AUDIO_PUBLISHED__COMPLETION_ACTIVE__WEB_EXPO_INTEGRATED') fail('proof result drift');

if (!process.exitCode) console.log('DAY002 SCALABILITY PASS: 705-word canon; generic registry; renderer reused; +1 DIS frozen server-side; audio published; completion active; Web/Expo integrated; authenticated/device QA pending');
